...
const char *formatLabel(){return "Smart Tank 540 compatibility / printer pass-through";}
...
bool connectWiFi(){if(config.ssid.isEmpty()){Serial.println("[WiFi] No saved SSID");return false;}WiFi.mode(WIFI_STA);WiFi.setHostname(HOSTNAME);WiFi.begin(config.ssid.c_str(),config.password.c_str());Serial.print("[WiFi] Connecting to ");Serial.println(config.ssid);const unsigned long deadline=millis()+20000UL;while(WiFi.status()!=WL_CONNECTED&&millis()<deadline){delay(250);Serial.print('.');}Serial.println();if(WiFi.status()!=WL_CONNECTED){Serial.printf("[WiFi] Connection failed, status=%d\n",(int)WiFi.status());WiFi.disconnect(false,false);return false;}Serial.print("[WiFi] Connected: ");Serial.println(WiFi.localIP());Serial.print("[WiFi] Hostname: ");Serial.println(HOSTNAME);return true;}
bool startConfigAP(){WiFi.mode(WIFI_AP_STA);WiFi.setHostname(HOSTNAME);if(!WiFi.softAP(AP_SSID,AP_PASSWORD,1,false,4)){Serial.println("[AP] Failed to start configuration AP");return false;}Serial.print("[AP] SSID: ");Serial.println(AP_SSID);Serial.print("[AP] Configure at http://");Serial.println(WiFi.softAPIP());return true;}

bool advertiseMobilePrinter(){
  MDNS.end();
  if(!MDNS.begin(HOSTNAME)){Serial.println("[mDNS] Failed to start mDNS responder");return false;}
  ...
  Serial.printf("[mDNS] %s -> _ipp._tcp:%u %s profile=%s\n",MobilePrintProfile::MODEL,MobilePrintProfile::IPP_PORT,MobilePrintProfile::IPP_PATH,MobilePrintProfile::PROFILE_NAME);
  return true;
}
