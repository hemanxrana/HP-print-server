...
void advertiseMobilePrinter() {
  ...
  Serial.println(".local advertising _ipp._tcp");
}
...
String dashboard() {
  ...
  if (!usbHost.device().attached || usbHost.interfaceCount() == 0) {
    ...
    usbOptions += "<form method='POST' action='/usb'><label><input type='radio' name='mode' value='auto' " + String(config.usbAuto ? "checked" : "") + "> Automatic — prefer standard bidirectional Printer Class (protocol 0x02)</label><br><br>";
    for (uint8_t i = 0; i < usbHost.interfaceCount(); ++i) {
      const UsbPrinterInterfaceInfo *p = usbHost.interfaceAt(i);
      if (!p) continue;
      const bool selected = !config.usbAuto && p->interfaceNumber == config.usbInterface && p->alternateSetting == config.usbAlt;
      usbOptions += "<label><input type='radio' name='mode' value='manual:" + String(p->interfaceNumber) + ":" + String(p->alternateSetting) + "' " + String(selected ? "checked" : "") + "> " + esc(usbInterfaceLabel(*p, p->interfaceNumber == usbHost.device().printer.interfaceNumber && p->alternateSetting == usbHost.device().printer.alternateSetting)) + "</label><br>";
    }
  ...
  String html = R"rawliteral(<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>HP Print Server</title><style>body{font-family:system-ui,Arial;max-width:800px;margin:24px auto;padding:0 16px;background:#f5f5f5;color:#222}section{background:#fff;padding:20px;margin:16px 0;border-radius:12px;box-shadow:0 2px 8px #0001}input{width:100%;box-sizing:border-box;padding:10px;margin:6px 0 14px;border:1px solid #aaa;border-radius:7px}input[type=radio]{width:auto;margin:6px 8px 6px 0}button{padding:11px 18px;border:0;border-radius:7px;background:#222;color:#fff;margin:5px 4px 5px 0}.status{padding:12px;background:#eee;border-radius:7px}code{word-break:break-all}</style></head><body><h1>HP Print Server</h1><section><h2>Status</h2><div class='status'>Wi-Fi: )rawliteral" + esc(wifi) + R"rawliteral(<br>IPP: )rawliteral" + String(ippServer.running() ? "ready" : "offline") + R"rawliteral(<br>USB: )rawliteral" + esc(usbState) + R"rawliteral(<br>Printer: )rawliteral" + esc(config.printerName) + R"rawliteral(<br>Model: )rawliteral" + esc(config.printerModel) + R"rawliteral(<br>Active jobs: )rawliteral" + String(printQueue.activeCount()) + " / " + String(MobilePrintQueue::MAX_JOBS) + R"rawliteral(<br>Retained jobs: )rawliteral" + String(printQueue.count()) + R"rawliteral(</div></section><section><h2>Wi-Fi</h2><form method='POST' action='/save'><label>SSID</label><input name='ssid' value=')rawliteral" + esc(config.ssid) + R"rawliteral(' maxlength='32'><label>Password</label><input type='password' name='password' placeholder='Leave blank to keep current password'><button>Save &amp; restart</button></form><p><a href='/scan'>Scan nearby networks</a></p></section><section><h2>Printer</h2><form method='POST' action='/save'><label>Printer name</label><input name='printerName' value=')rawliteral" + esc(config.printerName) + R"rawliteral('><label>Printer model</label><input name='printerModel' value=')rawliteral" + esc(config.printerModel) + R"rawliteral('><button>Save printer information</button></form></section><section><h2>USB Printer Interface</h2><p>Detected device: )rawliteral" + (usbHost.device().attached ? String("VID 0x") + String(usbHost.device().vid, HEX) + " / PID 0x" + String(usbHost.device().pid, HEX) : String("none")) + R"rawliteral(</p><p>Active: <b>)rawliteral" + esc(selected) + R"rawliteral(</b></p>)rawliteral" + usbOptions + R"rawliteral(<form method='POST' action='/usb/test'><button>Test Print</button></form><p>Test Print uses the production USB backend and selected interface. It is not a separate test sketch.</p></section><section><h2>Mobile printing</h2><p>Android Default Print Service / Mopria discovery: <b>DNS-SD + IPP</b></p><p>IPP endpoint: <code>)rawliteral" + printerUri() + R"rawliteral(</code></p><p>Accepted formats: PWG Raster, PCLm, PDF, JPEG, URF.</p><p>Jobs remain persistently pending until the USB backend transfers them successfully.</p><p><a href='/simulate'>Simulate the complete print pipeline</a></p></section></body></html>)rawliteral";
  return html;
}
...
void handleUsbSave() {
  ...
  if (mode == "auto") {
    ...
    if (second < 0) { configServer.send(400, "text/plain", "Invalid USB interface selection\n"); return; }
  ...
  if (!saveConfig()) { configServer.send(500, "text/plain", "USB configuration save failed\n"); return; }
...
void handleSimulate() {
  ...
  String body = "SIMULATION ONLY — no USB transfer is performed\n\n";
  body += "1. Smartphone discovers _ipp._tcp via DNS-SD\n";
  body += "2. Smartphone sends an IPP print job\n";
  body += "3. ESP32 validates document format and spools it to LittleFS\n";
  body += "4. Job enters STATE_PENDING\n";
  body += "5. USB backend waits for PRINTER_READY\n";
  body += "6. Selected interface: ";
  body += p ? ("IF " + String(p->interfaceNumber) + " ALT " + String(p->alternateSetting) + " OUT 0x" + String(p->bulkOut.address, HEX)) : "none";
  body += "\n7. Production backend claims that interface\n";
  body += "8. Document is streamed in USB Bulk OUT chunks\n";
  body += "9. Only after every USB transfer succeeds is the job marked STATE_COMPLETED\n";
  body += "10. On transfer failure the job is STATE_ABORTED; it is not retried in a tight loop\n";
...
void setup() {
  ...
  if (!usbPrinterBackend.begin()) Serial.println("[USB] Host start failed: " + usbPrinterBackend.statusReason());
  ...
  configServer.on("/simulate", HTTP_GET, handleSimulate);
  configServer.begin();
...
void loop() {
  ...
  // One persistent job is processed at a time. When USB is unavailable the
  // queue remains pending instead of entering a PROCESSING/ABORTED retry loop.
  if (usbPrinterBackend.online() && printQueue.hasPending()) {
    String error;
