...
// Smartphone-first ESP32-S3 print server.
// Network side: DNS-SD + IPP for Android/Mopria-style driverless printing.
// Printer side: queued document -> future USB/PCLm backend.
//
// The HP Smart Tank 520 is a USB-only model. HP specifies HP PCL 3 GUI and
// HP PCLm as its print languages, so the final backend must render/translate
// mobile print data to a format the USB printer actually accepts.
...
static constexpr const char *AP_PASSWORD = "configureme"; // development default
...
void advertiseMobilePrinter() {
  ...
  // Android Default Print Service/Mopria discovers driverless printers using
  // DNS-SD. We advertise only the protocol that is actually running.
  ...
  MDNS.addServiceTxt("ipp", "tcp", "pdl", "image/pwg-raster,image/urf,application/PCLm,application/pdf,image/jpeg");
...
bool handleMobileJob(const uint8_t *document, size_t length, const String &format,
  ...
  // Accept only formats the mobile profile explicitly exposes. The queue is
  // persistent, so a reboot does not silently lose an accepted mobile job.
  if (format != MobilePrintProfile::FORMAT_PWG &&
      format != MobilePrintProfile::FORMAT_PCLM &&
      format != MobilePrintProfile::FORMAT_PDF &&
      format != MobilePrintProfile::FORMAT_JPEG &&
      format != MobilePrintProfile::FORMAT_URF) {
    error = "Unsupported mobile document format: " + format;
    return false;
  }

  if (printQueue.hasJob()) {
    error = "Printer busy: one print job is already queued";
    return false;
  }
...
String dashboard() {
  String wifi = WiFi.status() == WL_CONNECTED
    ...
    : "Configuration required — AP 192.168.4.1";

  String html = R"rawliteral(<!doctype html><html><head>
<meta name='viewport' content='width=device-width,initial-scale=1'>
<title>HP Print Server</title><style>
body{font-family:system-ui,Arial;max-width:760px;margin:24px auto;padding:0 16px;background:#f5f5f5;color:#222}
section{background:#fff;padding:20px;margin:16px 0;border-radius:12px;box-shadow:0 2px 8px #0001}
input{width:100%;box-sizing:border-box;padding:10px;margin:6px 0 14px;border:1px solid #aaa;border-radius:7px}
button{padding:11px 18px;border:0;border-radius:7px;background:#222;color:#fff}.status{padding:12px;background:#eee;border-radius:7px}
</style></head><body><h1>HP Print Server</h1>
<section><h2>Status</h2><div class='status'>Wi-Fi: )rawliteral" + esc(wifi) + R"rawliteral(<br>
IPP: )rawliteral" + String(ippServer.running() ? "ready" : "offline") + R"rawliteral(<br>
Printer: )rawliteral" + esc(config.printerName) + R"rawliteral(<br>
Model: )rawliteral" + esc(config.printerModel) + R"rawliteral(<br>
Queued job: )rawliteral" + String(printQueue.hasJob() ? "yes" : "no") + R"rawliteral(</div></section>
<section><h2>Wi-Fi</h2><form method='POST' action='/save'>
...
<label>Printer name</label><input name='printerName' value=')rawliteral" + esc(config.printerName) + R"rawliteral('>
<label>Printer model</label><input name='printerModel' value=')rawliteral" + esc(config.printerModel) + R"rawliteral('>
<button>Save printer information</button></form></section>
<section><h2>Mobile printing</h2>
<p>Android Default Print Service / Mopria discovery: <b>DNS-SD + IPP</b></p>
<p>IPP endpoint: <code>)rawliteral" + printerUri() + R"rawliteral(</code></p>
<p>Accepted mobile formats: PWG Raster, PCLm, PDF, JPEG, URF.</p>
<p>The current firmware persistently queues the incoming job. The HP Smart Tank 520 USB/PCLm renderer and USB Host transport are the next hardware layer.</p>
</section></body></html>)rawliteral";
  return html;
}
...
void handleScan() {
  ...
  for (int i = 0; i < n; ++i) {
    String ssid = WiFi.SSID(i); if (ssid.isEmpty()) ssid = "(hidden)";
...
void setup() {
  ...
  if (wifi) {
    ...
    ippServer.begin(config.printerName, printerUri(), handleMobileJob);
  }
