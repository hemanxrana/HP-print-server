...
void loop(){configServer.handleClient();ippServer.poll();usbHost.poll();usbPrinterBackend.poll();static unsigned long lastStatus=0;if(millis()-lastStatus>5000){lastStatus=millis();bool wifi=WiFi.status()==WL_CONNECTED;if(wifi&&!lastWifiState){advertiseMobilePrinter();}lastWifiState=wifi;Serial.printf("[STATUS] WiFi=%d IP=%s USB=%d printer=%s led=%s\n",(int)WiFi.status(),WiFi.localIP().toString().c_str(),(int)usbHost.state(),usbPrinterBackend.stateText().c_str(),usbPrinterBackend.ledColor().c_str());}}
