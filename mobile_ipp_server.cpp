...
bool attr(uint8_t*o,size_t cap,size_t&pos,uint8_t tag,const char*n,const uint8_t*v,size_t len){
  size_t nl=strlen(n);if(nl>65535||len>65535||pos>cap||5+nl+len>cap-pos)return false;
  o[pos++]=tag;p16(o+pos,(uint16_t)nl);pos+=2;memcpy(o+pos,n,nl);pos+=nl;
  p16(o+pos,(uint16_t)len);pos+=2;if(len)memcpy(o+pos,v,len);pos+=len;return true;
}
...
bool range4(uint8_t*o,size_t c,size_t&p,const char*n,uint32_t lo,uint32_t hi){uint8_t b[8];p32(b,lo);p32(b+4,hi);return attr(o,c,p,0x33,n,b,8);}
bool resolution(uint8_t*o,size_t c,size_t&p,const char*n,uint32_t x,uint32_t y){uint8_t b[9];p32(b,x);p32(b+4,y);b[8]=3;return attr(o,c,p,0x32,n,b,9);}
...
void MobileIppServer::begin(const String&name,const String&uri,JobHandler handler,MobilePrintQueue*queue){
  ...
  printerPath_=slash>=0?uri.substring(slash):MobilePrintProfile::IPP_PATH;if(printerPath_.isEmpty())printerPath_=MobilePrintProfile::IPP_PATH;
  server_.begin();running_=true;Serial.printf("[IPP] Listening on TCP %u path=%s (streaming)\n",port_,printerPath_.c_str());
}
...
void MobileIppServer::handleClient(WiFiClient&c){
  ...
  // Keep transparent pass-through bounded by Content-Length. HP-style IPP
  // clients normally send Content-Length; accepting chunked would require
  // buffering/reframing the document and would defeat the streaming design.
  ...
  while(used<total){
    ...
    if(tag==0x01||tag==0x02||tag==0x04||tag==0x05){last="";continue;}
  ...
  Serial.printf("[IPP] op=0x%04X id=%lu format=%s document=%u bytes freeHeap=%u\n",op,(unsigned long)req,format.c_str(),(unsigned)documentLength,(unsigned)ESP.getFreeHeap());
  ...
  if(status==ST_OK){
    if(op==OP_GET_PRINTER_ATTRIBUTES){
      ...
      if(wants(requested,"printer-name"))namev(responseBuffer,RESPONSE_CAPACITY,w,"printer-name",model);
      if(wants(requested,"printer-info"))txt(responseBuffer,RESPONSE_CAPACITY,w,"printer-info",model);
      if(wants(requested,"printer-make-and-model"))txt(responseBuffer,RESPONSE_CAPACITY,w,"printer-make-and-model",model);
      if(wants(requested,"printer-uuid"))strv(responseBuffer,RESPONSE_CAPACITY,w,0x45,"printer-uuid",uuid);
      if(wants(requested,"printer-device-id"))txt(responseBuffer,RESPONSE_CAPACITY,w,"printer-device-id",deviceId);
      ...
      if(wants(requested,"print-color-mode-default"))kw(responseBuffer,RESPONSE_CAPACITY,w,"print-color-mode-default","color");
      if(wants(requested,"copies-supported"))range4(responseBuffer,RESPONSE_CAPACITY,w,"copies-supported",1,99);
      if(wants(requested,"media-supported")){kw(responseBuffer,RESPONSE_CAPACITY,w,"media-supported","iso_a4_210x297mm");kw(responseBuffer,RESPONSE_CAPACITY,w,"media-supported","na_letter_8.5x11in");kw(responseBuffer,RESPONSE_CAPACITY,w,"media-supported","na_legal_8.5x14in");kw(responseBuffer,RESPONSE_CAPACITY,w,"media-supported","na_index-4x6_4x6in");}
      if(wants(requested,"media-default"))kw(responseBuffer,RESPONSE_CAPACITY,w,"media-default","iso_a4_210x297mm");
      if(wants(requested,"media-type-supported")){kw(responseBuffer,RESPONSE_CAPACITY,w,"media-type-supported","stationery");kw(responseBuffer,RESPONSE_CAPACITY,w,"media-type-supported","photographic-glossy");kw(responseBuffer,RESPONSE_CAPACITY,w,"media-type-supported","photographic-matte");}
      if(wants(requested,"sides-supported"))kw(responseBuffer,RESPONSE_CAPACITY,w,"sides-supported","one-sided");
      if(wants(requested,"job-creation-attributes-supported")){kw(responseBuffer,RESPONSE_CAPACITY,w,"job-creation-attributes-supported","copies");kw(responseBuffer,RESPONSE_CAPACITY,w,"job-creation-attributes-supported","document-format");kw(responseBuffer,RESPONSE_CAPACITY,w,"job-creation-attributes-supported","media");kw(responseBuffer,RESPONSE_CAPACITY,w,"job-creation-attributes-supported","sides");kw(responseBuffer,RESPONSE_CAPACITY,w,"job-creation-attributes-supported","print-color-mode");}
      if(wants(requested,"printer-resolution-supported")){resolution(responseBuffer,RESPONSE_CAPACITY,w,"printer-resolution-supported",300,300);resolution(responseBuffer,RESPONSE_CAPACITY,w,"printer-resolution-supported",600,600);}
      if(wants(requested,"pages-per-minute"))integer(responseBuffer,RESPONSE_CAPACITY,w,"pages-per-minute",12);
      if(wants(requested,"finishings-supported"))enm(responseBuffer,RESPONSE_CAPACITY,w,"finishings-supported",3);
      if(wants(requested,"print-quality-supported")){enm(responseBuffer,RESPONSE_CAPACITY,w,"print-quality-supported",3);enm(responseBuffer,RESPONSE_CAPACITY,w,"print-quality-supported",4);}
