...
bool MobilePrintQueue::begin() {
  ...
  Preferences p;
  if (!p.begin(META_NS, true)) return true;
  ...
  if (!queued_) clear((String &)String(""));
  return true;
}
...
bool MobilePrintQueue::enqueue(const uint8_t *data, size_t length, const String &format, uint32_t &jobId, String &error) {
  ...
  if (LittleFS.exists(JOB_PATH) && !LittleFS.remove(JOB_PATH)) { LittleFS.remove(tmp); error = "Cannot replace spool file"; return false; }
  ...
  nextId_ = id;
  current_ = {id, length, format, STATE_PENDING};
  queued_ = true;
  jobId = id;
  return true;
}
...
bool MobilePrintQueue::getJob(uint32_t id, JobInfo &info) const {
  if (!queued_ || current_.id != id) return false;
  info = current_;
  return true;
}
...
bool MobilePrintQueue::setState(State state, String &error) {
  ...
  bool ok = p.putUChar("state", state) > 0;
  p.end();
