...
static bool connectClient(WiFiClient& c, const PrintTarget& t, uint16_t fallback, uint32_t timeout) {
  c.setTimeout(timeout / 1000 + 1);
  uint16_t port = t.port ? t.port : fallback;
  if (t.address != IPAddress(0, 0, 0, 0)) return c.connect(t.address, port, timeout);
...
static bool attr(uint8_t* b, size_t cap, size_t& pos, uint8_t tag, const char* name, const uint8_t* v, size_t n) {
...
static bool buildIppHeader(uint8_t* b, size_t cap, size_t& length, const String& uri,
  ...
  if (!sattr(b, cap, p, 0x47, "attributes-charset", "utf-8") ||
      !sattr(b, cap, p, 0x48, "attributes-natural-language", "en") ||
      !sattr(b, cap, p, 0x45, "printer-uri", uri) ||
      !sattr(b, cap, p, 0x42, "requesting-user-name", user.isEmpty() ? String("esp32") : user) ||
      !sattr(b, cap, p, 0x49, "document-format", fmt)) return false;
  b[p++] = 3; length = p; return true;
}
...
static bool readIppResponseStatus(WiFiClient& c, uint16_t* status, uint32_t timeoutMs) {
  if (status) *status = 0xFFFF;
  uint8_t h[8]; size_t got = 0; unsigned long deadline = millis() + timeoutMs;
  while (got < sizeof(h) && (long)(deadline - millis()) > 0) {
    if (!c.available()) { delay(1); continue; }
    int n = c.read(h + got, sizeof(h) - got); if (n > 0) got += (size_t)n;
  }
...
static bool sendIppStream(const PrintTarget& target, const String& printerUri, const String& user,
...
bool IppTransport::sendStream(const PrintTarget& target, const String& printerUri, const String& user,
                              const String& documentFormat, Stream& source, size_t documentLength,
                              uint32_t timeoutMs, uint16_t* ippStatus) {
  return sendIppStream(target, printerUri, user, documentFormat, source, documentLength, timeoutMs, ippStatus);
}

bool IppTransport::send(const PrintTarget& target, const String& printerUri, const String& user,
                        const String& documentFormat, const uint8_t* document, size_t documentLength,
                        uint32_t timeoutMs, uint16_t* ippStatus) {
  ...
  return sendStream(target, printerUri, user, documentFormat, source, documentLength, timeoutMs, ippStatus);
}
