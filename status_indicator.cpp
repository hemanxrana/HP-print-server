...
// Keep the RGB LED active without flooding Serial Monitor with identical status lines.
// Set to 1 only when detailed periodic [STATUS] snapshots are needed.
...
constexpr uint32_t RMT_HZ = 10000000UL;
...
#if PRINT_STATUS_LOG
void logStatus(){static uint32_t lastLog=0;static uint8_t lastState=255;static int lastWifi=-1;static uint8_t lastJobs=255;int wifi=(int)WiFi.status();uint8_t jobs=printQueue.activeCount();uint8_t s=scenario();if(millis()-lastLog<5000UL&&s==lastState&&wifi==lastWifi&&jobs==lastJobs)return;Serial.printf("[STATUS] WiFi=%d",wifi);if(wifi==WL_CONNECTED)Serial.printf(" IP=%s",WiFi.localIP().toString().c_str());Serial.printf(" USB=%u jobs=%u",(unsigned)usbHost.state(),(unsigned)jobs);const UsbDeviceInfo&d=usbHost.device();if(d.attached){Serial.printf(" printer=VID:0x%04X PID:0x%04X addr:%u",d.vid,d.pid,d.address);if(d.product.length())Serial.printf(" product=\"%s\"",d.product.c_str());if(d.manufacturer.length())Serial.printf(" manufacturer=\"%s\"",d.manufacturer.c_str());if(d.serial.length())Serial.printf(" serial=\"%s\"",d.serial.c_str());if(usbHost.selectedInterface()){const auto&p=*usbHost.selectedInterface();Serial.printf(" IF=%u ALT=%u protocol=0x%02X OUT=0x%02X IN=0x%02X",p.interfaceNumber,p.alternateSetting,p.protocol,p.bulkOut.address,p.bulkIn.address);}}else Serial.print(" printer=none");if(usbHost.lastError().length())Serial.printf(" error=\"%s\"",usbHost.lastError().c_str());Serial.printf(" led=%s\n",colorHex(colors[s]).c_str());lastLog=millis();lastState=s;lastWifi=wifi;lastJobs=jobs;}
#else
void logStatus(){}
#endif
